...
;B[gd];W[gc];B[jk];W[fl];B[cm];W[ji];B[kd];W[le];B[jc];W[jf];B[mf];W[jb]
...
--	tex.sprint("Start: " .. start)
--	tex.sprint("Stop: " .. stop)
end
...
function play_move (p, c)		--> if p not defined -> pass
	...
	if displayMove(Board.MoveNumber) then
		...
		if p then
			...
			if writeLabel(Board.MoveNumber) then Board.Label[x][y] = Board.MoveNumber end
			testDeadStones(x, y)
		end
	end
end
...
function parseSGF(SGFstring)
	...
	Bproperty = lpeg.P("B[") * lpeg.Cmt(shortparam, blackmove) * lpeg.P("]")	* emptychar^0
	Wproperty = lpeg.P("W[") * lpeg.Cmt(shortparam, whitemove) * lpeg.P("]") * emptychar^0

	property = Bproperty + Wproperty
